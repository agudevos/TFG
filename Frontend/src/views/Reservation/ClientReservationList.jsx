...
const ClientReservationsList = () => {
  ...
  const fetchReservations = async () => {
    try {
      ...
      const response = await getFromApi(`reservations/client/`);
    ...
    <div className="container mx-auto px-4 py-14">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center">
          <div>
            <h2 className="flex items-center flex-wrap text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-white">
        ...
        <div className="flex flex-col sm:flex-row gap-2 ml-auto">
          <button 
            onClick={handleNew}
            disabled={isLoading}
            className="w-full sm:w-32 px-4 py-2 bg-green-500 hover:bg-green-600 disabled:bg-gray-400 text-white rounded-lg transition-colors duration-300 flex items-center justify-center"
          >
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-8 text-center">
          ...
          <button
            onClick={handleNew}
            className="mt-4 px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg transition-colors duration-300"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reservations && reservations.map((reservation) => (
            <SimpleReservationItem 
              ...
              showClient={false} // No mostrar cliente ya que es específico para un cliente
            />
          ))}
        </div>
      )}
